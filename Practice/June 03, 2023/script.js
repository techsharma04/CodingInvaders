...
function displayProducts(products) {
  products.forEach((product) => {
    let individualProduct = `<div class="product">
          <div class="productOuter">
            <img src=${product.fields.image[0].thumbnails.small.url} />
            <div class="productInner">
              <h2>${product.fields.name}</h2>
              <h3><em>${product.fields.category}</em></h3>
              <h3>${product.fields.company}</h3>
...
function filterProducts(category) {
  ...
  if (category.toLowerCase() === "all") {
    ...
    filteredProducts = productData.filter(
      (product) =>
        product.fields.category.toLowerCase() === category.toLowerCase()
    );
  }
  content = "";
